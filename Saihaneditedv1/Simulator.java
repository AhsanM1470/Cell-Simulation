        ...
        for (int col = 0; col < field.getWidth(); col++) {
          Location location = new Location(row, col, 0);
            ...
            if (mycoRand.nextDouble() <= MYCOPLASMA_ALIVE_PROB) {
                //adds a myco to a particular location
    ...
    private void whitePopulate(){
        ...
        for (int row = 0; row < field.getDepth(); row++) {
            for (int col = 0; col < field.getWidth(); col++) {
                ...
                if (whiteRand.nextDouble() <= WHITE_BLOOD_CELL_ALIVE_PROB ) {
                    WhiteBloodCell white = new WhiteBloodCell(field, location, Color.CYAN);
                    cells.add(white);
                }
                else {
//                    white.setDead();
